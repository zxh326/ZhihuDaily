...
def getJson(count, head, url, parameter):
    ...
    for old_item in data['items']:
        ...
        if old_item['format'] != 'word':
            ...
            item['type'] = old_item['format']
        try:
            item['user'] = {'name': str(old_item['user']['login']), 'image': str(
                old_item['user']['medium'])}
        except:
            item['user'] = 'null'
        if old_item.__contains__('hot_comment'):
            item['hashot'] = str('True')
            item['hot_comment'] = {'content': str(old_item['hot_comment']['content']), 'user': {
                'name': (old_item['hot_comment']['user']['login'])}}
        else:
